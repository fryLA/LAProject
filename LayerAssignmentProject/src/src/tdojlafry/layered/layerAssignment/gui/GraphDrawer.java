...
public class GraphDrawer extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Edge edge : currEdges) {
            ...
            Line2D line =
                    new Line2D.Double(n1.getPosition().x, n1.getPosition().y, n2.getPosition().x, n2.getPosition().y);
        ...
        for (Node node : currNodes) {
            Rectangle2D rect =
                    new Rectangle2D.Double(node.getPosition().x, node.getPosition().y, nodeWidth, nodeHeight);
        ...
        for (int i = 0; i < layerCnt; i++) {
            ...
            Rectangle2D rect = new Rectangle2D.Double(PADDING + i * layerWidth, heightOfMiniGraph + PADDING, layerWidth,
                    layerHeight);
            g2.setColor(Color.BLUE);
            g2.draw(rect);
        }
